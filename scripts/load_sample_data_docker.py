#!/usr/bin/env python3
"""
Docker-based sample data loader for ChatGPT Web App.
This script runs inside the Docker container to load sample data.
"""
...
# Add the backend directory to the path (for Docker container)
...
async def load_sample_data():
    ...
    # Use environment variables for database connection (Docker container style)
    database_url = os.getenv('DATABASE_URL')
    if not database_url:
        database_url = (
            f"postgresql+asyncpg://{os.getenv('POSTGRES_USER', 'chatgpt_user')}:"
            f"{os.getenv('POSTGRES_PASSWORD', 'chatgpt_password')}@"
            f"{os.getenv('POSTGRES_SERVER', 'localhost')}:"
            f"{os.getenv('POSTGRES_PORT', '5432')}/"
            f"{os.getenv('POSTGRES_DB', 'chatgpt_webapp')}"
        )
    ...
    # Sample data paths (inside Docker container)
